...
public class ParallelExecution {
	...
	private static void mergingAsync() {
		Observable.merge(getDataAsync(1), getDataAsync(2)).toBlocking().forEach(each -> {
	...
	private static void mergingSync() {
		...
		Observable.merge(getDataSync(1), getDataSync(2)).toBlocking().forEach(each -> {
				System.out.print(Thread.currentThread().getName());
				System.out.println(each);
			}
		);
	...
	private static void mergingSyncMadeAsync() {
		...
		Observable.merge(getDataSync(1).subscribeOn(Schedulers.io()), getDataSync(2).subscribeOn(Schedulers.io())).toBlocking().forEach(
			System.out::println);
	}
